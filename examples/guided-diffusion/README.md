# ER-SDE-Solver for guided-diffusion

This is the example code for ER-SDE-Solver applying to  [guided-diffusion]((https://github.com/openai/guided-diffusion)). The code is heavily based on [guided-diffusion codebase]((https://github.com/openai/guided-diffusion)).

The example command can be found in `run.sh`. Before running the command, it is necessary to download pre-trained model checkpoints based on the corresponding command. 

Just like this original repo, we provide two scripts for generating images, namely `er_sde_sample.py` without classifier guidance and `classifier_er_sde_sample.py` with classifier guidance. The stats file for evaluating the FID is the same as the original repo. 
